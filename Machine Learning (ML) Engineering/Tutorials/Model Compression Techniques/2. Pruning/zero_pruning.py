"""
Zero Pruning for Model Compression

This script implements zero pruning to reduce the number of active parameters in a fully connected neural network. 
By analysing activation levels of neurons, this technique prunes low-importance connections, leading to a more 
compact model while maintaining acceptable accuracy.
...
5. Apply zero pruning based on activation thresholds.
...
def prune_network(net, activations, threshold):
    """
    Applies zero pruning to the network based on activation thresholds.
    """
...
def main():
    ...
    for threshold in tqdm(thresholds):
        ...
        new_total_params = sum(p.numel() for p in pruned_net.parameters())
        size_reduction = 1 - (new_total_params / original_total_params)
        
        results.append([threshold, accuracy * 100, original_total_params, new_total_params, size_reduction])
    
    results_df = pd.DataFrame(results, columns=["Threshold", "Accuracy", "Original Params", "New Params", "Size Reduction"])
    print(results_df)
